...
function CCBCustomLoad(ccbFile, ccbControlName, customData)
	local proxyData = customData and customData or {};
	ccb[ccbControlName] = proxyData;
	proxyData.proxy = CCBProxy:create()
	local layer = CCBuilderReaderLoad(ccbFile, proxyData.proxy, proxyData);
	
	return proxyData, layer;
end
