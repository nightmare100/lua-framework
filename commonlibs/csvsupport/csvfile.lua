...
function Csv:ctor(filepath, mode, separator)
	self:__init(filepath, mode, separator)
end
-- initializer
...
function Csv:write(a)
   local res, msg = self.file:write(tocsv(a, self.separator),"\n")
...
function Csv:writeall(a)
   for i,entry in ipairs(a) do
      local res, msg = self.file:write(tocsv(entry, self.separator),"\n")
