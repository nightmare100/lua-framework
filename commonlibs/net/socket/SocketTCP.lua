...
local SOCKET_RECONNECT_MAXTIMES = 3;	-- socket reconnect times
...
function SocketTCP:_onConnected()
	...
	print("[onConnectedHandler!]")
	if self.connectTimeTickScheduler then scheduler.unscheduleGlobal(self.connectTimeTickScheduler) end


	-- start to read TCP data
	self.tickScheduler = scheduler.scheduleGlobal(handler(self, self.onData), SOCKET_TICK_TIME)
end

function SocketTCP:onData()
--	while true do
...
function SocketTCP:_reconnect(__immediately)
	...
	if self.reconnectTime > SOCKET_RECONNECT_MAXTIMES then
